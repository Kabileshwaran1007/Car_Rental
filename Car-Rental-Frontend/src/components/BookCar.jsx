...
const BookCar = () => {
  ...
  const [searchParams] = useSearchParams(); // For payment confirmation
...
const cityCostMap = {
  ...
  const confirmBooking = async () => {
    ...
    try {
      ...
      // const sessionId = response.data.session_id; // Adjust based on your API response
      // navigate(`/success?session_id=${sessionId}`); // Redirect to SuccessPage with session_id
  ...
  useEffect(() => {
    const checkPaymentStatus = async () => {
        try {
            ...
            // Make sure the alert happens before the navigation
            setTimeout(() => {
                navigate("/");  // Navigate to the homepage after alert
            }, 500);  // Delay to ensure the alert is displayed first
        }
    };
...
// Change the export statement to default
export default BookCar;

// Keep the SuccessPage export as named if you need it separately
export { SuccessPage };